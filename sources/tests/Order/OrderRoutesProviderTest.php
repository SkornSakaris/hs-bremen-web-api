...
class OrderRoutesProviderTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function registerIndexRoute()
    {
        ...
        $controllerFactory->get('/{orderId}', 'service.order:getDetails')->shouldBeCalled();
        $controllerFactory->post('/', 'service.order:createOrder')->shouldBeCalled();
        $controllerFactory->put('/{orderId}', 'service.order:changeOrder')->shouldBeCalled();
